...
# Monitoring setup
LOG_PATH = './logs'
LOG_DISCRYPTION = 'FINAL_' + str(TARGET)
...
class MPC_env(gym.Env):
    ...
    def __init__(self,config):
        ...
        if not self.Debuging:
            print("Not debugging!")
            self.simulator = FDTD_kernel(self.xclbin, self.Nx, self.Ny, self.Nt, self.MPC_inst.dt, self.wl, device_id = (self.worker_index - 1))
        else:
            print("Debug mode!")
    ...
    def reset(self, rand = False, target = TARGET):
        if rand:
        ...
        if not self.Debuging:
            ...
            E1, E2 = self.observe()
            self.E_total = E1 + E2
            self.E_total = self.E_total / 0.85 # the total transition is measured as 0.92
...
trainer = PPOTrainer(config=config_train,logger_creator=custom_log_creator(os.path.expanduser(LOG_PATH), 'MPC_' + LOG_DISCRYPTION))
